...
/** Track 0: tempo (FF 51 03) and time signature (FF 58 04) only. */
 ...
 */
function noteTrack(role, notes) {
  const settings = TRACK_SETTINGS[role] || { name: role, program: 0, channel: 0 };
  ...
  const body = [...vlq(0), ...meta(0x03, ascii(settings.name)), ...vlq(0), 0xc0 | ch, settings.program & 0x7f];
  let lastTick = 0;
 ...
 * unknown roles go last. Returns a Uint8Array.
 */
export function encodeMidi(ir) {
  ...
  const ordered = [
    ...TRACK_ORDER.map((role) => tracks.find((t) => t.role === role)).filter(Boolean),
    ...tracks.filter((t) => !TRACK_ORDER.includes(t.role)),
  ];
  const chunks = [tempoTrack(ir), ...ordered.map((t) => noteTrack(t.role, t.notes))];
...
export function trackNames(ir) {
  ...
  ].map((role) => (TRACK_SETTINGS[role] || { name: role }).name);
}
