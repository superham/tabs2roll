...
import { classifyGlyph, isMusicGlyph } from "./digits.js";
...
export function readSystem(bitmap, system, options = {}) {
  ...
  // A mark much bigger than its neighbours is a time signature or a part
  // number, not a fret. Judged against the staff's own numbers rather than a
  // fixed size, because every score is drawn at its own scale.
  const typical = median(candidates.map((c) => c.component.height)) || spacing;
  const marks = [];
  let unreadable = 0;
  for (const candidate of candidates) {
    if (candidate.component.height > typical * 1.6) continue;
    const read = classifyGlyph(candidate.component, options);
...
export function joinDigits(marks, typical) {
  ...
  for (let i = 0; i < marks.length; i++) {
    ...
    // Only 1 and 2 start a fret number: there is no 24th-and-a-bit fret, and
    ...
    const joins = leads && next && next.string === mark.string && next.char !== "x" && next.x0 - mark.x1 <= typical * DIGIT_JOIN;
    if (joins) {
      const fret = Number(mark.char) * 10 + Number(next.char);
      if (fret <= MAX_FRET) {
        events.push({ x: (mark.x0 + next.x1) / 2, string: mark.string, fret, glyph: mark.char + next.char, score: Math.min(mark.score, next.score) });
        i++;
        continue;
      }
    }
    events.push({ x: mark.cx, string: mark.string, fret: Number(mark.char), glyph: mark.char, score: mark.score });
  }
